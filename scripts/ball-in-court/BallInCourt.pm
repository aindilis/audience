...
# check all emails in order to determine who I have to email and who
# should email me.  also, if people haven't responded to my emails in
# a long while, queue them (based on the rated importance of the
# email) for a re-query.

# analyze the use of "Re: " syntax to determine who is responding, as
# in Gmail.

# come up with a list of people's changing emails over time, an their
# multiple emails, or just do this in bbdb

use ASConsole::Corpus::Message;
use BallInCourt::Contact;
use BOSS::Config;
...
use Class::MethodMaker
  ...
  [
   ...
   SentMailDir Contacts Last /

  ];

sub init {
  print("Initializing BallInCourt\n");
  my ($self,%args) = @_;
  my $specification = "
...
";
  ...
  $UNIVERSAL::systemdir = ConcatDir(Dir("internal codebases"),"audience","scripts","ball-in-court");
...
sub Connect {
  ...
  print("Done initializing BallInCourt\n");
}
...
sub AddMessage {
  ...
  foreach my $addr (Mail::Address->parse($m->{Party})) {
    ...
    if (! exists $self->Contacts->{$contact}) {
      $self->Contacts->{$contact} = BallInCourt::Contact->new(Name => $contact);
    }
